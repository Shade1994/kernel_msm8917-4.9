...
/* huaqin add compatible awk9 by limengxia at 2019/11/6 start */
struct pa_ctrl_struct  ppa_ctrl;
int pa_ctrl_register(struct pa_ctrl_struct* cust)
{
	ppa_ctrl.pa_off = cust->pa_off;
	ppa_ctrl.spr_on = cust->spr_on;
	ppa_ctrl.rcv_on = cust->rcv_on;
	return 0;
}
EXPORT_SYMBOL(pa_ctrl_register);
int card_smartpa = 0;
EXPORT_SYMBOL(card_smartpa);
/* huaqin add compatible awk9 by limengxia at 2019/11/6 end */
...
static struct wcd_mbhc_config mbhc_cfg = {
	...
	/* huaqin add for HS60-25 bringup headphone function by xudayi at 2019/07/11 start */
	.key_code[1] = KEY_VOLUMEUP,
	.key_code[2] = KEY_VOLUMEDOWN,
	.key_code[3] = 0,
	/* huaqin add for HS60-25 bringup headphone function by xudayi at 2019/07/11 end */
	.key_code[4] = 0,
	.key_code[5] = 0,
	.key_code[6] = 0,
	.key_code[7] = 0,
...
static int enable_spk_ext_pa(struct snd_soc_codec *codec, int enable)
{
	struct snd_soc_card *card = codec->component.card;
	struct msm_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	/* HS60 add HS60-196 set  pa mode  2 by xudayi at 2019/07/23 start */
	int pa_mode = 2;
	/* HS60 add HS60-196 set  pa mode  2 by xudayi at 2019/07/23 end */
	/* HS70 modify for awk9 bring up by limengxia at 2019/11/6 start  */
	if(card_smartpa == 5){
		pr_err("%s:  external xudayi card_smartpa %d\n", __func__, card_smartpa);
	}else{
		if (!gpio_is_valid(pdata->spk_ext_pa_gpio)) {
	...
	if(card_smartpa == 5){
		pr_err("%s:  external speaker PA aw87359\n", __func__);
		if (enable) {
			ppa_ctrl.spr_on();
		} else {
			ppa_ctrl.pa_off();
	...
	/* HS70 modify for awk9 bring up by limengxia at 2019/11/6 end  */
	return 0;
}

/* HS60 add HS60-98 bringup awinic pa for HAC by xudayi at 2019/07/25 start */
int is_ext_hac_gpio_support(struct platform_device *pdev,
			struct msm_asoc_mach_data *pdata)
{
...
/* HS60 add HS60-29 integrate nxp smartpa cal func by xudayi at 2019/07/23 start */
...
/* HS60 add HS60-29 integrate nxp smartpa cal func by xudayi at 2019/07/23 end */
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	/* HS60 add HS60-98 bringup awinic pa for HAC by xudayi at 2019/07/25 start */
	SOC_ENUM_EXT("HAC_SWITCH", msm_snd_enum[7],
...
#undef S
	 ...
	 * 360-680 == Button 3
	 */
	/* HS60  add for P191111-07088 compatible samsung headset by xudayi at 2019/11/30 start */
	btn_low[0] = 100;
	btn_high[0] = 100;
	/* HS60  add for P191111-07088 compatible samsung headset by xudayi at 2019/11/30 end */
	/* huaqin add for HS60-25 bringup headphone function by xudayi at 2019/07/11 start */
	btn_low[1] = 220;
	btn_high[1] = 220;
	btn_low[2] = 450;
...
static int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	/* HS60 add HS60-373 voicecall speaker no sound after suspend by xudayi at 2019/08/08 start */
	snd_soc_dapm_ignore_suspend(dapm, "Ext Spk");
	/* HS60 add HS60-373 voicecall speaker no sound after suspend by xudayi at 2019/08/08 end */
...
/* HS60 add AR-ZQL1695-01-58 exchange i2c address by xudayi at 2019/07/30 start */
static struct snd_soc_dai_link_component fs16xx_codecs[] ={
        {
                .name     = "fs16xx.2-0036",
                .of_node  = NULL,
                .dai_name = "fs16xx-aif",
...
/* HS60 add AR-ZQL1695-01-58 exchange i2c address by xudayi at 2019/07/30 end */

static struct snd_soc_dai_link_component aw8864_codecs[] ={
        {
                .name     = "aw8864_smartpa",
                .of_node  = NULL,
                .dai_name = "aw8864-aif",
...
/* HS70-24 bring up aw smartpa by xuyuqing at 2019/10/31 start */
...
/* HS70-24 bring up aw smartpa by xuyuqing at 2019/10/31 end */

//xuyuqing
static struct snd_soc_dai_link_component tas2562_codecs[] = {
	{
		.of_node = NULL,
		.dai_name = "tas2562 ASI1",
		.name = "tas2562.2-004c",
	},
};
//xuyuqing

static struct snd_soc_dai_link msm_mi2s_be_dai_links_aw[] = {
	{
		.name = LPASS_BE_QUIN_MI2S_RX,
		.stream_name = "Quinary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.4",
		.platform_name = "msm-pcm-routing",
		.codecs = aw8864_codecs,
		.num_codecs = 1,
		.no_pcm = 1,
		.dpcm_playback = 1,
...
//xuyuqing
static struct snd_soc_dai_link msm_mi2s_be_dai_links_tas2562[] = {
	{
		.name = LPASS_BE_QUIN_MI2S_RX,
		.stream_name = "Quinary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.4",
		.platform_name = "msm-pcm-routing",
		.codecs = tas2562_codecs,
		.num_codecs = 1,
		.no_pcm = 1,
		.dpcm_playback = 1,
		.id = MSM_BACKEND_DAI_QUINARY_MI2S_RX,
...
static struct snd_soc_dai_link msm_mi2s_be_dai_links_fs[] = {
	{
		.name = LPASS_BE_QUIN_MI2S_RX,
		.stream_name = "Quinary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.4",
...
static struct snd_soc_dai_link msm8952_dai[] = {
	...
	{/* hw:x,5 */
		/* huaqin add HS60-30 add fm route for speaker by xudayi at 2019/07/17 start */
		.name = "INT4 MI2S_RX Hostless",
		.stream_name = "INT4 MI2S_RX Hostless",
		.cpu_dai_name = "INT4_MI2S_RX_HOSTLESS",
		/* huaqin add HS60-30 add fm route for speaker by xudayi at 2019/07/17 end */
						...
						struct device *dev)
{
	...
	/* huaqin add bringup nxp & foursemi smartpa by xudayi at 2019/07/15 start */
	if(card_smartpa == 1){
		msm8952_quin_dai_link[0] = msm_mi2s_be_dai_links_fs[0];
		msm8952_dai[58] = msm_mi2s_be_dai_links_fs[1];
	}else if(card_smartpa == 2){
		msm8952_quin_dai_link[0] = msm_mi2s_be_dai_links_tfa[0];
		msm8952_dai[58] = msm_mi2s_be_dai_links_tfa[1];
	//xuyuqing
	}else if(card_smartpa ==3){
		msm8952_quin_dai_link[0] = msm_mi2s_be_dai_links_tas2562[0];
		msm8952_dai[58] = msm_mi2s_be_dai_links_tas2562[1];
	//xuyuqing
	/* HS70-24 bring up aw smartpa by xuyuqing at 2019/10/31 start */
	}else if(card_smartpa ==4){
		msm8952_quin_dai_link[0] = msm_mi2s_be_dai_links_aw882xx[0];
		msm8952_dai[58] = msm_mi2s_be_dai_links_aw882xx[1];
	/* HS70-24 bring up aw smartpa by xuyuqing at 2019/10/31 end */
	/* HS70 bring up awk9 by limengxia at 2019/11/6 start */
	}else if(card_smartpa == 5){
		printk(KERN_ERR "card_smartpa is 5,aw87359\n");
	/* HS70 bring up awk9 by limengxia at 2019/11/6 end */
	}else if(card_smartpa == 6){
		msm8952_quin_dai_link[0] = msm_mi2s_be_dai_links_aw[0];
		msm8952_dai[58] = msm_mi2s_be_dai_links_aw[1];
	}
	/* huaqin add bringup nxp & foursemi smartpa by xudayi at 2019/07/15 end */
...
parse_mclk_freq:
	...
	/* huaqin add bringup nxp & foursemi smartpa by xudayi at 2019/07/15 start */
	if(card_smartpa == 1)
		card->name = "sdm439-sku1-snd-card-fs";
	else if(card_smartpa == 2)
		card->name = "sdm439-sku1-snd-card-tfa";
	else if(card_smartpa == 6)
		card->name = "sdm439-sku1-snd-card-aw8864";
	/* huaqin add bringup nxp & foursemi smartpa by xudayi at 2019/07/15 end */

	/* huaqin add compatible ti & aw smartpa by xuyuqing at 2019/09/30 start */
	if(card_smartpa == 3)
		card->name = "msm8953-snd-card-mtp-ti";
	else if(card_smartpa == 4)
		card->name = "msm8953-snd-card-mtp-aw";
	/* huaqin add compatible awk9 by xuyuqing at 2019/10/20 start */
	else if(card_smartpa == 5)
		card->name = "msm8953-snd-card-mtp-awk9";
	/* huaqin add compatible awk9 by xuyuqing at 2019/10/20 end */
	/* huaqin add compatible ti & aw smartpa by xuyuqing at 2019/09/30 end */
	/* initialize timer */
